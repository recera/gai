...
- Streaming: SSE parser emits `content` deltas; tool call arguments are coalesced by index and emitted at `finish_reason == "tool_calls"`
...
- Compatible with OpenAI‑like chat shapes; streaming currently emulated (use SimulatedStreaming middleware)

## General notes

- Unsupported options are ignored; use `ProviderOpts` for provider‑specific features
- Use `Headers` to set gateway headers
